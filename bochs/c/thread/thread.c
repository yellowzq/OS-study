...
void init_thread(struct task_struct* pthread, char* name, int prio) {
    ...
    pthread->pid = allocate_pid(); // 分配一个 pid
 ...
 * 就是为其预留pcb的，因此 pcb 地址为 0xc009f000，不需要通过 get_kernel_page 另分配一页*/
    main_thread = running_thread();
    init_thread(main_thread, "main", 31);

/* main 函数是当前线程，当前线程不在 thread_ready_list 中，所以值将其加在 thread_all_list 中*/
