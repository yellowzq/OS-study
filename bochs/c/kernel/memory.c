...
struct arena {
    ...
    uint32_t cnt;                   // large为true时，cnt表示的是页框数。否侧cnt表示空闲mem_block数量
    bool large;                      // 是否是大块内存
};
...
/* 得到虚拟地址 vaddr 对应的 pte指针*/
uint32_t* pte_ptr(uint32_t vaddr){
    /* 先访问到页表自己 + \
     * 在用页目录项pde（页目录内页表的索引）作为pte的索引访问到页表 + \
     * 再用 pte 的索引作为页内偏移*/
...
/* 页表中添加虚拟地址_vaddr 与物理地址 _page_phyaddr 的映射*/
static void page_table_add(void* _vaddr, void* _page_phyaddr) {
    ...
    * 否则会引发 page_fault。因此在*pde 为 0 时，*pte 只能出现在下面 else 语句块中的*pde 后面。
    ***********************************************************/
...
void* get_user_pages(uint32_t pg_cnt) {
    lock_acquire(&user_pool.lock); // 申请用户内存池的锁
    void* vaddr = malloc_page(PF_USER, pg_cnt);
    memset(vaddr, 0, pg_cnt * PG_SIZE);
    lock_release(&user_pool.lock); // 释放用户内存池的锁
    return vaddr;
}
...
void* get_a_page(enum pool_flags pf, uint32_t vaddr) {
    struct pool* mem_pool = pf & PF_KERNEL ? &kernel_pool : &user_pool;
    lock_acquire(&mem_pool->lock); // 申请内存池的锁
    ...
    page_table_add((void*)vaddr, page_phyaddr); // 在页表中做映射
    lock_release(&mem_pool->lock); // 释放内存池的锁
    return (void*)vaddr;
}
...
uint32_t addr_v2p(uint32_t vaddr) {
    uint32_t* pte = pte_ptr(vaddr);
    /* (*pte) 的值是页表所在的物理页框地址，
    * 去掉其低 12 位的页表项属性+虚拟地址vaddr的低12位*/
    return ((*pte & 0xfffff000) + (vaddr & 0x00000fff)); // 返回物理地址
}
...
static void mem_pool_init(uint32_t all_mem){
    ...
    //页表大小 = 1 页的页目录表 + 第 0 和第 768 个页目录项指向通一个页表 + 第 769～1022 个页目录项共指向 254 个页表，共 256 个页框
    uint32_t used_mem = page_table_size + 0x100000;     // 0x100000 为低端 1MB 内存
    uint32_t free_mem = all_mem - used_mem;
    uint16_t all_free_pages = free_mem / PG_SIZE;
    // 1 页为 4KB，不管总内存是不是 4k 的倍数b，对于以页为单位的内存分配策略，不足 1 页的内存不用考虑了
    ...
    * ************************************************/
    ...
    lock_init(&kernel_pool.lock); // 初始化内核内存池的锁
    lock_init(&user_pool.lock);   // 初始化用户内存池的锁
...
void block_desc_init(struct mem_block_desc* desc_array) {
    ...
    for (desc_idx = 0; desc_idx < DESC_CNT; desc_idx++) {
        ...
        /* 通过不断左移，block_size 最终变为 16,32,64,128,256,512,1024 */
...
void* sys_malloc(uint32_t size){
    ...
    if(size>1024){
        ...
        if(list_empty(&descs[desc_idx].free_list)){
            ...
            /* 将arena拆分成内存块加入到mem_block_desc的free_list中 */
...
/* 去掉页表中虚拟地址 vaddr 的映射，只去掉vaddr对应的pte */
...
void mfree_page(enum pool_flags pf, void* _vaddr, uint32_t pg_cnt) {
    ...
    /* 确保嗲释放的物理内存池在低端1MB+1KB大小的页目录+1KB大小的页表地址范围外*/
    ...
    if (pg_phy_addr >= user_pool.phy_addr_start) { // 用户物理内存池
        ...
        while(page_cnt < pg_cnt) {
            ...
            pfree(pg_phy_addr);           // 将物理页框归还给物理内存池
            page_table_pte_remove(vaddr); // 去掉页表中虚拟地址 vaddr 的映射

            page_cnt++;
        }
        vaddr_remove(pf, _vaddr, pg_cnt); // 清除虚拟地址的位图中的相应位
    } else { // 内核物理内存池
        vaddr -= PG_SIZE;
        while(page_cnt < pg_cnt) {
            vaddr += PG_SIZE;
...
void sys_free(void* ptr) {
    ...
    if (a->desc == NULL && a->large == true) { // 大块内存
        mfree_page(PF, a, a->cnt);
    } else { // 小块内存
        list_append(&a->desc->free_list, &b->free_elem); // 将内存块回收到内存块描述符的 free_list 中
        a->cnt++;
        
        /* 如果arena中的内存块都被回收，就释放arena */
