...
/* 判断 bit_idx 位是否为1，若为1，则返回true，否则返回false*/
...
/* 若在位图数组范围内的某自己内找到了空闲位，在该字节内逐字节内逐位比较，返回空闲位的索引*/
    int idx_bit = 0;
...
/* 将位图btmp 的 bit_idx为设置为 value*/
void bitmap_set(struct bitmap* btmp, uint32_t bit_idx, int8_t value){
    ASSERT((value==0) || (value == 1));
    uint32_t byte_idx = bit_idx/8;      //向下取整用于索引数组下标
    uint32_t bit_odd = bit_idx%8;       //取余用于索引数组内的位

/* 一般都会用个 0x1 这样的数对字节中的位操作，将1任意移动后再取反，或者先取反在位移，可用来对位置0操作*/
    if(value){                          //如果value为1
